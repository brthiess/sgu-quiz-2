  ...
  <div class="landing-container" :class="{ 'quiz-active': active }">
    <div class="title-button-container" :class="{ 'quiz-active': active }">
      <div class="title-container" :class="{ 'quiz-active': active }">
        <h1 class="title" :class="{ 'quiz-active': active }">
          Science or Fiction
        </h1>
      </div>
      <h4 class="subtitle" :class="{ 'quiz-active': active }">
        Compete against the rogues with a series of Science or Fiction questions
        and see how you stack up!
      </h4>
      <ul class="rogue-list" :class="{ 'quiz-active': active }">
        <li class="rogue-list-item">
          <img
            class="rogue-list-item-image"
            :class="{ 'quiz-active': active }"
            src="/images/steve.jpg"
          /><span class="rogue-name">Steve</span>
        </li>
        <li class="rogue-list-item">
          <img
            class="rogue-list-item-image"
            :class="{ 'quiz-active': active }"
            src="/images/jay.png"
          /><span class="rogue-name">Jay</span>
        </li>
        <li class="rogue-list-item">
          <img
            class="rogue-list-item-image"
            :class="{ 'quiz-active': active }"
            src="/images/bob.jpg"
          /><span class="rogue-name">Bob</span>
        </li>
        <li class="rogue-list-item">
          <img
            class="rogue-list-item-image"
            :class="{ 'quiz-active': active }"
            src="/images/cara.jpg"
          /><span class="rogue-name">Cara</span>
        </li>
        <li class="rogue-list-item">
          <img
            class="rogue-list-item-image"
            :class="{ 'quiz-active': active }"
            src="/images/evan.jpg"
          /><span class="rogue-name">Evan</span>
        </li>
      </ul>
      <div
        class="begin-quiz-button-container"
        :class="{ 'quiz-active': active }"
      >
        <button
  ...
  methods: {
    ...
    getEpisodeJson(episodeNumber) {
      ...
      return new Promise(function(resolve, reject) {
        self
          .makeApiRequest("/json/" + episodeNumber + ".json")
          .then(function(data) {
            resolve(data);
          })
...
.title-container {
  margin: 30px 0;
  transition: all 0.6s, border-bottom-left-radius 0.3s,
